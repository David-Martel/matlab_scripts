...
classdef VLC < matlab.mixin.SetGet
  ...
  methods
    ...
    function this = VLC()
      ...
      this.requestURL = sprintf('http://%s:%d/request.json',this.ipaddr4,this.Port);
      try
        ...
        if isempty(retry) && ~isempty([strfind(e.message,'java.net.ConnectException') strfind(e.message,'java.net.SocketTimeoutException')])
          args = sprintf('--extraintf http --http-host %s --http-port %d --http-password "%s" --http-src "%s"',...
            this.ipaddr4,this.Port,this.password,fileparts(mfilename('fullpath')));
          if ispc
            if exist("C:\Program Files\VideoLAN\VLC\vlc.exe",'file')==2
