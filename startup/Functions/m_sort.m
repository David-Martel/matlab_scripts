...
if ~user_finished
    for ch=ch_start_sort:max_ch
        ...
        if height(tbl)>10
            ...
            fspace_all = pca75(tbl_aligned);
            instruction_display();
            close all;
            sort_code{ch} = user_sort(fspace_all,ch,titlestring);
...
function sort_code = user_sort(fspace_all,ch,titlestring)
...
while count>0
    ...
    set(gca,'xtick',0,'xticklabel','pc1',...
        'ytick',0,'yticklabel','pc2',...
        'ztick',0,'zticklabel','pc3')
    h=get(gca,'children');
    grid on
    brush on
    k=0;
