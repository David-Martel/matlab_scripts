...
%         p.Results.findext = replace(p.Results.findext,'.','');
        if iscell(p.Results.findext)
            dirlist = fullfile(dirName,'**',strcat('*.',p.Results.findext));
            for iter = 1:length(p.Results.findext)
                temp_dir = dir(dirlist{iter});
                if iter==1
                    dirData = temp_dir;
                else
                    dirData = cat(1,dirData,temp_dir);
                end
            end
        else
