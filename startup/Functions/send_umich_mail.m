function send_umich_mail(to,subject,theMessage,attachments,varargin)
...
% Argument parsing.
narginchk(2,4);
if (ischar(to) || isStringScalar(to)) && to ~= ""
    to = {to};
end
if (nargin < 3)
    theMessage = '';
...
if (nargin < 4)
    attachments = [];
elseif (ischar(attachments) || isStringScalar(attachments)) && attachments ~= ""
...
% Set sender.
% name_idx = contains(varargin,'from_name');
% if any(name_idx)
%     name_idx = find(name_idx)+1;
%     from_name = varargin{name_idx};
% % else
% %     from_name = "The Banquet People";
% end


from = 'tinn.trial+noreply2022@umich.edu';
from_name = "Tinnitus Trial";

msg.setFrom(getInternetAddress(from,from_name));

msg.addRecipient(getRecipientTypeTo(msg),...
       getInternetAddress(from,from_name));

% msg.setFrom(getInternetAddress('damartel@umich.edu','David Martel'));

% Set recipients.
for i = 1:numel(to)
    msg.addRecipient(getRecipientTypeCC(msg), ...
        getInternetAddress(to{i}));
end

% msg.addRecipient(getRecipientTypeBCC(msg),getInternetAddress('simonli+csd@umich.edu','Superman Li'));
% 
...
%===============================================================================
function internetAddress = getInternetAddress(from,varargin)
%getInternetAddress Instantiate an InternetAddress object.

try
    
    if ~isempty(varargin)
        from_name = varargin{1};
        internetAddress = javax.mail.internet.InternetAddress(from,from_name);
    else
...
%===============================================================================
function recipientTypeBCC = getRecipientTypeBCC(msg)
%getRecipientTypeTo Return the static RecipientType.TO.
...
rt = java.lang.Class.forName('javax.mail.Message$RecipientType', false, cl);
% Returns a Field object pointint to TO.
field = rt.getField('BCC');
% Gets the static instance of TO.
recipientTypeBCC = field.get([]);

%===============================================================================
function recipientTypeCC = getRecipientTypeCC(msg)
%getRecipientTypeTo Return the static RecipientType.TO.
...
rt = java.lang.Class.forName('javax.mail.Message$RecipientType', false, cl);
% Returns a Field object pointint to TO.
field = rt.getField('CC');
% Gets the static instance of TO.
recipientTypeCC = field.get([]);
